...
كل push إلى فرع `main` يشغّل workflow الهاتف تلقائيًا، ويرفع Release جديدًا بصيغة `mobile-v1.0.<run-number>`. التطبيق يقرأ أحدث Release من GitHub عند فتحه، ويقارن النسخة المثبتة، ثم يعرض شاشة تحديث إجبارية إذا وجد إصدار أعلى. التنزيل يستخدم Android DownloadManager داخل مجلد التطبيق، لذلك يستمر بعد إغلاق QuizSpace ويستعيد حالته ونسبته عند فتحه مرة أخرى. بعد اكتمال الكاش فقط يظهر زر التثبيت؛ عند الضغط يُفحص SHA-256 ثم يفتح مثبت Android لتأكيد التثبيت فوق النسخة الحالية.
