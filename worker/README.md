...
2. Set `OPENROUTER_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and `ALLOWED_ORIGIN` (for example, `https://quiz-space-app.pages.dev`) as Wrangler secrets. OpenRouter handles model fallback internally; the Worker does not call DeepSeek, Groq, or OpenAI directly.
