...
2. Set `GROQ_API_KEY`, `OPENROUTER_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and `ALLOWED_ORIGIN` (for example, `https://quiz-space-app.pages.dev`) as Wrangler secrets. Groq's `llama-3.3-70b-versatile` is the primary text model; OpenRouter remains the fallback and handles multimodal document requests.
