...
-- badge_tier/name_color cosmetic update RPC (plan-validated server-side)
CREATE OR REPLACE FUNCTION update_badge_and_name_color(p_badge_tier TEXT, p_name_color TEXT)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    END IF;
    UPDATE users SET badge_tier = p_badge_tier, name_color = p_name_color WHERE uid = auth.uid()::text;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
