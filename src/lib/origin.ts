...
export function getPublicQuizShareUrl(quizId: string, quizTitle: string, isChallenge = false): string {
  ...
  // Keep the AI Worker host private. The clean share host renders quiz-specific
  // metadata server-side, then sends human visitors to the actual app.
  const shareHost = 'https://quiz-space-share.pages.dev';
  ...
  return `${shareHost}/share/quiz?${query}`;
}
