import { readFileSync } from 'node:fs';
import { migrationSql } from '../testHelpers/migrationSql';
import { resolve } from 'node:path';
import { describe, expect, it } from 'vitest';

const migrationPath = 'supabase/migrations/20260831_notification_lesson_and_platform_settings_access.sql';

const weeklyTaskMigrationPath = 'supabase/migrations/20260834_weekly_task_notification_type.sql';

describe('notification and platform-settings schema repair', () => {
  it('allows the lesson notification emitted by the classroom lesson trigger', () => {
    const migration = migrationSql('supabase/migrations/20260831_notification_lesson_and_platform_settings_access.sql');

    expect(migration).toContain("'lesson'");
    expect(migration).toContain('notifications_type_check');
  });

  it('allows the weekly-task notification emitted after a successful reward claim', () => {
    const migration = migrationSql('supabase/migrations/20260834_weekly_task_notification_type.sql');

    expect(migration).toContain("'weekly_task'");
    expect(migration).toContain('notifications_type_check');
  });

  it('keeps platform settings publicly readable but does not grant client writes', () => {
    const migration = migrationSql('supabase/migrations/20260831_notification_lesson_and_platform_settings_access.sql');

    expect(migration).toContain('GRANT SELECT ON TABLE public.platform_settings TO anon, authenticated;');
    expect(migration).not.toMatch(/GRANT\s+(?:ALL|INSERT|UPDATE|DELETE)\s+ON\s+TABLE\s+public\.platform_settings/i);
  });
});
