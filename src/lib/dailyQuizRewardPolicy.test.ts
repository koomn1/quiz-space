...
const rewardMigrationPath = 'supabase/migrations/20260836_repair_daily_quiz_rewards.sql';
const compatibilityMigrationPath = 'supabase/migrations/20260837_extend_legacy_daily_quiz_completion_records.sql';
const timestampMigrationPath = 'supabase/migrations/20260838_set_daily_completion_timestamps.sql';
const retryMigrationPath = 'supabase/migrations/20260839_fix_daily_completion_retry_ambiguity.sql';
...
describe('daily quiz reward persistence policy', () => {
  it('records private daily completions and rewards them atomically without public quiz foreign keys', () => {
    const migration = migrationSql('supabase/migrations/20260836_repair_daily_quiz_rewards.sql');
  ...
  it('extends the legacy completion marker before reward-aware records rely on its fields', () => {
    const migration = migrationSql('supabase/migrations/20260837_extend_legacy_daily_quiz_completion_records.sql');
  ...
  it('keeps legacy timestamp defaults and retry updates safe after the first reward', () => {
    const timestampMigration = migrationSql('supabase/migrations/20260838_set_daily_completion_timestamps.sql');
    const retryMigration = migrationSql('supabase/migrations/20260839_fix_daily_completion_retry_ambiguity.sql');
