...
export default function Security({ lang }: SecurityProps) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const deviceFormatted = parseUserAgent(ua);

      let userLocation = Intl.DateTimeFormat().resolvedOptions().timeZone || 'محيط المستخدم المحلي';
      if (userLocation.includes('/')) {
        const parts = userLocation.split('/');
        userLocation = `${parts[1].replace(/_/g, ' ')} (${parts[0]})`;
      }
      ...
      if (dbSessions.length > 0) {
        setSessions(dbSessions.map((s, idx) => ({
          ...
          device: s.device,
          rawUa: s.userAgent,
          ip: s.ipAddress || 'الجلسة النشطة',
          location: s.location || userLocation,
          lastActive: s.lastActive ? new Date(s.lastActive).toLocaleString(isAr ? 'ar-EG' : 'en-US') : (isAr ? 'نشط الآن' : 'Active Now'),
          current: s.id === currentSessionId || idx === 0
        })));
      } else {
        // Fallback for current active session display
        const { data } = await supabase.auth.getSession();
        setSessions([{
          id: currentSessionId || (data.session?.access_token.substring(0, 16) || 'sess_current'),
          device: deviceFormatted,
          rawUa: ua,
          ip: 'الجلسة النشطة الحالية',
          location: userLocation,
          lastActive: isAr ? 'نشط الآن' : 'Active Now',
          current: true
        }]);
      }
    } catch (err) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        ...
        setPasswordFeedback(isAr ? 'فشل تحديث كلمة المرور. يرجى التأكد من الحقول والمحاولة لاختبار لاحقاً.' : 'Failed to update password. Please check fields and retry.');
      }
    } finally {
