...
function SectionNav({ section, onNavigate, lang }: Pick<MotivationHubPageProps, 'section' | 'onNavigate' | 'lang'>) {
  ...
  return (
    <div className="flex flex-wrap gap-2 rounded-3xl border border-slate-200 bg-white/85 p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900/85">
      {items.map((item) => (
        <button
          key={item.id}
          type="button"
          onClick={() => onNavigate(item.id)}
          className={`flex items-center gap-2 rounded-2xl px-4 py-2.5 text-xs font-black transition-all ${section === item.id ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-lg shadow-violet-500/20' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}
        >
...
function Overview({ rewards, status, onNavigate, onCheckIn, isCheckingIn, lang }: { rewards: Rewards; status: any; onNavigate: (section: MotivationSection) => void; onCheckIn: () => void; isCheckingIn: boolean; lang: 'ar' | 'en' }) {
  const t = labels[lang];
  const cards = [
    { id: 'motivation-lucky' as const, title: t.lucky, text: t.spinHint, image: imageUrl('lucky_wheel'), gradient: 'from-fuchsia-600 to-violet-600', icon: <Gift className="h-5 w-5" /> },
    { id: 'motivation-brain' as const, title: t.brain, text: lang === 'ar' ? 'سؤال يتغير يومياً وتحليل آمن للإجابة.' : 'A daily question with secure answer analysis.', image: imageUrl('brain_challenge'), gradient: 'from-cyan-600 to-blue-600', icon: <Brain className="h-5 w-5" /> },
    { id: 'motivation-store' as const, title: t.store, text: lang === 'ar' ? 'اشترِ إطارات مميزة وباقات نقاط.' : 'Buy premium frames and points bundles.', image: imageUrl('mystery_box'), gradient: 'from-amber-500 to-orange-600', icon: <Store className="h-5 w-5" /> },
  ];
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 to-indigo-100 p-5 dark:border-violet-900/50 dark:from-violet-950/30 dark:to-indigo-950/20">
          ...
          <p className="mt-1 text-xs font-bold text-violet-700/70 dark:text-violet-300/70">{lang === 'ar' ? 'إجابات صحيحة في تحدي العقل' : 'correct Brain Challenge answers'}</p>
          <button type="button" onClick={() => onNavigate('motivation-brain')} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-violet-600 px-3 py-2.5 text-xs font-black text-white transition hover:bg-violet-700"><Brain className="h-4 w-4" />{t.brain}</button>
        </div>
        <div className="rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-50 to-cyan-100 p-5 dark:border-sky-900/50 dark:from-sky-950/30 dark:to-cyan-950/20">
          <div className="mb-4 flex items-center justify-between"><span className="rounded-2xl bg-white/80 p-3 text-2xl shadow-sm dark:bg-slate-900/50">⚡</span><span className="text-xs font-black text-sky-700 dark:text-sky-300">{lang === 'ar' ? 'رصيدك الحالي' : 'Your balance'}</span></div>
          <div className="text-4xl font-black text-sky-700 dark:text-sky-300">{rewards.points.toLocaleString()}</div>
          <p className="mt-1 text-xs font-bold text-sky-700/70 dark:text-sky-300/70">{t.points}</p>
          <button type="button" onClick={() => onNavigate('motivation-store')} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-sky-600 px-3 py-2.5 text-xs font-black text-white transition hover:bg-sky-700"><Store className="h-4 w-4" />{t.store}</button>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        {cards.map((card) => (
          <button key={card.id} type="button" onClick={() => onNavigate(card.id)} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white text-start shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900">
            <div className="relative h-32 overflow-hidden"><img src={card.image} alt="" loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className={`absolute inset-0 bg-gradient-to-r ${card.gradient} opacity-50`} /><div className="absolute bottom-3 start-3 rounded-xl bg-white/90 p-2 text-slate-800 shadow-lg dark:bg-slate-950/80 dark:text-white">{card.icon}</div></div>
            <div className="p-4"><h3 className="flex items-center gap-2 text-sm font-black text-slate-900 dark:text-white">{card.title}<ChevronRight className="h-4 w-4 rtl:rotate-180" /></h3><p className="mt-2 text-xs leading-6 text-slate-500 dark:text-slate-400">{card.text}</p></div>
          </button>
        ))}
      </div>
    </div>
...
function StorePanel({ userId, isPremium, planName, rewards, onRewardsChanged, lang }: { userId: string; isPremium?: boolean; planName?: string; rewards: Rewards; onRewardsChanged: () => void; lang: 'ar' | 'en' }) {
  ...
  const renderCard = (item: StoreItem) => { const locked = planRank < rankFor(item.min_plan); const isOwned = owned(item.id); return <div key={item.id} className="flex flex-col rounded-3xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900"><div className={`relative mb-4 flex h-28 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br ${item.css_class === 'frame-fire' ? 'from-orange-500 to-rose-600' : item.css_class === 'frame-crystal-luxe' ? 'from-cyan-400 to-indigo-600' : item.css_class === 'frame-star-crown' ? 'from-amber-400 to-yellow-600' : item.css_class === 'frame-diamond-comet' || item.css_class === 'frame-diamond-crown' ? 'from-sky-400 to-violet-700' : 'from-violet-500 to-fuchsia-600'}`}><div className="h-16 w-16 rounded-full border-4 border-white/90 bg-slate-900/35 shadow-[0_0_0_5px_rgba(255,255,255,.22)]" />{locked && <div className="absolute inset-0 flex items-center justify-center bg-slate-950/55 text-white"><Lock className="h-6 w-6" /></div>}</div><div className="flex-1"><h3 className="text-sm font-black text-slate-900 dark:text-white">{lang === 'ar' ? item.name_ar : item.name}</h3><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{lang === 'ar' ? item.description_ar : item.description}</p></div><div className="mt-4 flex items-center justify-between gap-2"><span className="text-xs font-black text-amber-600 dark:text-amber-300">{locked ? t.diamond : item.price_points === 0 ? t.owned : `${item.price_points.toLocaleString()} ${t.points}`}</span><button type="button" disabled={locked || busy === item.id} onClick={() => isOwned ? useFrame(item) : buyFrame(item)} className="rounded-xl bg-violet-600 px-3 py-2 text-[11px] font-black text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-45">{busy === item.id ? <Loader2 className="h-4 w-4 animate-spin" /> : isOwned ? (activeFrameId === item.id ? (lang === 'ar' ? 'مفعل' : 'Active') : (lang === 'ar' ? 'استخدام' : 'Use')) : t.buy}</button></div></div>; };
...
export default function MotivationHubPage({ userId, userName, isPremium, planName, lang, section, onNavigate }: MotivationHubPageProps) {
  ...
  return <main className="mx-auto w-full max-w-6xl px-4 pb-12 pt-4 sm:px-6 lg:px-10" dir={lang === 'ar' ? 'rtl' : 'ltr'}><div className="mb-6 flex flex-col gap-4 rounded-[2rem] border border-violet-200 bg-gradient-to-br from-violet-600 via-indigo-600 to-sky-600 p-6 text-white shadow-xl shadow-violet-500/10 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.22em] text-white/70"><Sparkles className="h-4 w-4" />QuizSpace Rewards</div><h1 className="text-3xl font-black sm:text-4xl">{pageTitle}</h1><p className="mt-2 max-w-2xl text-sm leading-7 text-white/80">{t.subtitle}{userName ? ` · ${userName}` : ''}</p></div><RewardPill rewards={rewards} lang={lang} /></div><SectionNav section={section} onNavigate={onNavigate} lang={lang} /><div className="mt-6">{section === 'motivation' && <Overview rewards={rewards} status={status} onNavigate={onNavigate} onCheckIn={checkIn} isCheckingIn={isCheckingIn} lang={lang} />}{section === 'motivation-lucky' && <LuckyWheelPanel onRewardsChanged={onRewardsChanged} lang={lang} />}{section === 'motivation-brain' && <BrainChallengePanel onRewardsChanged={onRewardsChanged} lang={lang} />}{section === 'motivation-store' && <StorePanel userId={userId} isPremium={isPremium} planName={planName} rewards={rewards} onRewardsChanged={onRewardsChanged} lang={lang} />}</div></main>;
}
