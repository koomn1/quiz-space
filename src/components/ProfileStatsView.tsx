...
import {
  ...
  BadgeTier, NameColorKey, BadgeColorKey
} from './PremiumNameTag';
...
export default function ProfileStatsView({
  ...
  const [infoCustomId, setInfoCustomId] = React.useState('');
  ...
  const [infoBadgeTier, setInfoBadgeTier] = React.useState<BadgeTier>('none');
  const [infoNameColor, setInfoNameColor] = React.useState<NameColorKey>('default');
  const [infoSelectedBadgeColor, setInfoSelectedBadgeColor] = React.useState<BadgeColorKey>('blue');
  ...
  const handleOpenEditInfo = () => {
    ...
    setInfoCustomId(profileData?.customId || '');
    setCustomIdError('');
    ...
    // these are what UserBadge actually renders on the profile header.
    setInfoBadgeTier((profileData?.badgeTier as BadgeTier) || 'none');
    setInfoNameColor((profileData?.nameColor as NameColorKey) || 'default');
    setInfoSelectedBadgeColor((profileData?.badgeColor as BadgeColorKey) || 'blue');
    setBadgeSaveError('');
  ...
  const handleSaveInfoSubmit = async (e: React.FormEvent) => {
    ...
    // Validate username (@customId) before saving
    const trimmedCustomId = infoCustomId.trim().toLowerCase();
    if (trimmedCustomId) {
      const isValidFormat = /^[a-z0-9_]{3,20}$/.test(trimmedCustomId);
      if (!isValidFormat) {
        setCustomIdError(
          isAr
            ? 'اسم المستخدم يجب أن يكون بين 3 و20 حرفًا (حروف إنجليزية صغيرة، أرقام، أو _)'
    ...
    try {
      ...
      await saveUserProfile(
        ...
        infoBio,
        serializedLocation,
        undefined, // badgeSymbol: legacy/unused field, left untouched
        undefined, // badgeColor: managed exclusively via updateBadgeAndNameColor below
        trimmedCustomId || undefined
      );

      // Badge tier/name color/badge color are plan-validated server-side and
      // live in a separate RPC — only call it for premium users who can
      // actually have a badge_tier other than 'none'.
      if (isUserPremium) {
        try {
          await updateBadgeAndNameColor(profileData.userId, infoBadgeTier, infoNameColor, infoSelectedBadgeColor);
        } catch (badgeErr: any) {
          console.error('Failed to save badge/name color:', badgeErr);
          setBadgeSaveError(
            isAr ? 'تعذّر حفظ إعدادات الشارة، حاول مرة أخرى' : 'Could not save badge settings, please try again'
          );
        }
      }
      ...
      if (err?.message?.includes('duplicate') || err?.code === '23505') {
        setCustomIdError(
          isAr ? 'اسم المستخدم هذا مستخدم بالفعل، جرّب اسمًا آخر' : 'This username is already taken, try another one'
        );
      }
    } finally {
  ...
  // Mirrors planNameToTier() in lib/db.ts — must stay in sync so the tiers/colors
  // offered here match what update_badge_and_name_color() will actually allow server-side.
  const planTier: 'Free' | 'Silver' | 'Gold' | 'Diamond' = (() => {
    const p = (profileData?.planName || '').toLowerCase();
    ...
    return isUserPremium ? 'Silver' : 'Free';
  })();
          ...
          >
            {creations.length === 0 ? (
              <div className="py-12 text-center bg-slate-50 dark:bg-slate-900/20 border border-slate-200/80 dark:border-[#1f2648]/40 rounded-2xl space-y-2">
                ...
                <p className="text-[10px] text-slate-400 dark:text-slate-500">{isAr ? 'أي جولة ستقوم بتصنيعها عبر الذكاء الاصطناعي ستعولم وتظهر للطلاب هنا.' : 'Any quiz you generate with AI from images will appear directly here.'}</p>
              </div>
            ) : (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              ...
              <form onSubmit={handleSaveInfoSubmit} className="space-y-4">
                ...
                {/* 1.5 Username (@customId) Input */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300 block">
                    {isAr ? 'اسم المستخدم (@username)' : 'Username (@username)'}
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={infoCustomId}
                      onChange={(e) => {
                        setInfoCustomId(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''));
                        if (customIdError) setCustomIdError('');
                      }}
                      className={`w-full bg-[#0a0a0c] border rounded-xl pl-7 pr-4 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none text-right font-mono transition-all shadow-[inset_0_1px_3px_rgba(0,0,0,0.4)] ${
                        customIdError ? 'border-red-500 focus:border-red-500' : 'border-slate-800 focus:border-purple-500'
                      }`}
                  ...
                  {customIdError ? (
                    <p className="text-[10px] text-red-400 font-bold">{customIdError}</p>
                  ) : (
                    <p className="text-[10px] text-slate-500 font-semibold">
                      {isAr
                        ? '٣-٢٠ حرفًا إنجليزيًا صغيرًا أو أرقام أو "_" فقط. اتركه فارغًا للاحتفاظ بالمعرف التلقائي.'
                        : '3-20 characters: lowercase letters, numbers, or "_" only. Leave empty to keep the auto-generated ID.'}
                    </p>
                  )}
                </div>
                ...
                {/* 5. Real Verification Badge Selection — writes to badge_tier/name_color/badge_color
                     via the plan-validated update_badge_and_name_color RPC (see PremiumNameTag.tsx) */}
                <div className="pt-2 border-t border-slate-850 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <label className="text-xs font-black text-white">{isAr ? 'علامة التوثيق وشارة التميز' : 'Verification Badge'}</label>
                  </div>

                  {!isUserPremium ? (
                    <p className="text-[10px] text-slate-500 font-bold text-center py-2">
                      {isAr ? 'اشترك في باقة مدفوعة لفتح شارات التوثيق وألوان الاسم المخصصة.' : 'Subscribe to a paid plan to unlock verification badges and custom name colors.'}
                    </p>
                  ) : (
                    <div className="space-y-3">
                      {/* Badge tier */}
                      <div className="grid grid-cols-4 gap-2">
                        {availableBadgeTiers(planTier).map((tier) => (
                          <button
                            key={tier}
                            type="button"
                            onClick={() => setInfoBadgeTier(tier)}
                            className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all border text-center gap-1 ${
                              infoBadgeTier === tier
                          ...
                          >
                            <div className="h-6 flex items-center justify-center">
                              {tier === 'none' ? (
                                ...
                                <UserBadge tier={tier as any} size="sm" showTooltip={false} badgeColor={infoSelectedBadgeColor} />
                              )}
                            </div>
                            <span className="text-[8px] font-bold truncate block w-full">
                      ...
                      {infoBadgeTier !== 'none' && (
                        <div className="space-y-1.5">
                          <p className="text-[10px] text-slate-400 font-bold">{isAr ? 'لون الشارة' : 'Badge color'}</p>
                          <div className="flex flex-wrap gap-2">
                            {availableBadgeColors(planTier).map((c) => (
                              <button
                                key={c}
                                type="button"
                                onClick={() => setInfoSelectedBadgeColor(c)}
                                title={isAr ? BADGE_COLOR_PRESETS[c].labelAr : BADGE_COLOR_PRESETS[c].labelEn}
                                className={`w-6 h-6 rounded-full border-2 transition-all ${infoSelectedBadgeColor === c ? 'border-white scale-110' : 'border-transparent opacity-70'}`}
                                style={{ background: BADGE_COLOR_PRESETS[c].gradient }}
                              />
                            ))}
                          </div>
                        </div>
                      ...
                      {/* Name color */}
                      <div className="space-y-1.5">
                        <p className="text-[10px] text-slate-400 font-bold">{isAr ? 'لون الاسم' : 'Name color'}</p>
                        <div className="flex flex-wrap gap-1.5">
                          {availableNameColors(planTier).map((nc) => (
                            <button
                              key={nc}
                              type="button"
                              onClick={() => setInfoNameColor(nc)}
                              className={`px-2 py-1 rounded-lg border text-[10px] font-bold transition-all ${
                                infoNameColor === nc ? 'border-white bg-slate-800' : 'border-slate-800 bg-slate-950/40 hover:bg-slate-900'
                              }`}
                              style={NAME_COLOR_PRESETS[nc].style}
                            >
                              {isAr ? NAME_COLOR_PRESETS[nc].labelAr : NAME_COLOR_PRESETS[nc].labelEn}
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      {badgeSaveError && (
                        <p className="text-[10px] text-red-400 font-bold">{badgeSaveError}</p>
                      )}
                    </div>
                  )}
                </div>
